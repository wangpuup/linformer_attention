...
class LinformerAttention(nn.Module):
    ...
    def __init__(self, n_head, n_feat, rank, n_attn):
    ...
    def forward(self, query, key, value, mask):
        ...
        LinformerAttention.projection_matrix = nn.Parameter(torch.Tensor(self.h, self.linformer_k, seq_len))
        torch.nn.init.normal_(LinformerAttention.projection_matrix, std = 0.02)
        self.E = LinformerAttention.projection_matrix.to(q.device) 

        k = torch.matmul(self.E, k)
        v = torch.matmul(self.E, v)

        dot = torch.matmul(q, torch.transpose(k, -2, -1))
        dot = dot / math.sqrt(self.d_k)

        self.attn = nn.functional.softmax(dot, dim = -1)

        x = torch.matmul(self.attn, v)
